...
def get_recommendations(user_id, feature_type=None, count=5):
    ...
    for item_index in candidate_items:
        ...
        if model is None:
            prediction = item_features[item_index].get('cf_score', 0) + item_features[item_index].get('content_score', 0)
        else:
    ...
    if feature_type == 'breakfast':
        ...
        recommendations = [
            ...
            any(keyword in r['recipe_name'].lower() for keyword in breakfast_keywords)
        ]
    elif feature_type == 'lunch':
        lunch_keywords = ['trưa']
        recommendations = [
            r for r in recommendations if r.get('meal_time') == 'lunch' or \
            any(keyword in r['recipe_name'].lower() for keyword in lunch_keywords)
        ]
    elif feature_type == 'dinner':
        dinner_keywords = ['tối', 'chiều']
        recommendations = [
            r for r in recommendations if r.get('meal_time') == 'dinner' or \
            any(keyword in r['recipe_name'].lower() for keyword in dinner_keywords)
        ]
    elif feature_type == 'easy':
        recommendations = [r for r in recommendations if r['difficulty'] == 'Dễ']
...
def age_based_recommendations():
    user_id = request.args.get('user_id')
    age_group = request.args.get('age_group')  # children, teenagers, adults, elderly
    ...
    try:
        ...
        if age_group == 'children':
            ...
            age_filtered_recommendations = [r for r in recommendations if r['difficulty'] == 'Dễ']
            ...
            age_filtered_recommendations = [r for r in recommendations if r['difficulty'] in ['Dễ', 'Trung bình']]
...
def meal_recommendations():    user_id = request.args.get('user_id')
    ...
    try:
        ...
        recommendations = get_recommendations(user_id, feature_type=meal_type, count=count)
...
def meal_plans():
    ...
    try:
        ...
        breakfast_recs = get_recommendations(user_id, feature_type='breakfast', count=6)
        lunch_recs = get_recommendations(user_id, feature_type='lunch', count=6)
        dinner_recs = get_recommendations(user_id, feature_type='dinner', count=6)
